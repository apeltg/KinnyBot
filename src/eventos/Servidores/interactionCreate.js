...
module.exports = async (client, interaction) => {
  ...
  if(interaction.isAutocomplete()) {
    if(interaction.commandName === 'play') {
      ...
      if(interaction.options.getFocused().includes('https://') || interaction.options.getFocused().includes('http://')) return;
  ...
  if (interaction.isCommand()) {
    ...
    if (comandoInfo) {
      ...
      if(comandoInfo.config.categoria === "mod" && !interaction.member.permissions.has('ADMINISTRATOR')) return interaction.reply('Você não tem permissão para executar esse comando!')
      comandoInfo.run(client, interaction, args).catch(error => {
        ...
        interaction.reply({ content: 'Infelizmente ocorreu um erro ao executar esse comando! Já foi reportado para o criador o motivo do erro! \n \nUnfortunately there was an error executing this command! The reason for the error has already been reported to the creator!', ephemeral: true })
      })
