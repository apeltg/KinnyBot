...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        let cd = await db.repsc.findOne({membro: !message.author ? message.user.id:message.author.id}) 
        if(!lan) {
        if(cd) {
        ...
        if(mencao.id === !message.author ? message.user.id:message.author.id) return message.reply('Você não pode da reputação para a si mesmo!')
        ...
        if(!membro) {
            await db.reps.create({
                id: mencao.id,
                reps: rep,
                membro: !message.author ? message.user.id:message.author.id
            })
            await db.repsc.create({membro: !message.author ? message.user.id:message.author.id, cooldown: Date.now()})
            message.reply(`Você deu uma reputação para ${mencao} agora ele tem 1 reputação!`)
                ...
                await db.reps.findOneAndUpdate({id: mencao.id}, {reps: membro.reps + rep, membro: !message.author ? message.user.id:message.author.id})
                await db.repsc.create({membro: !message.author ? message.user.id:message.author.id, cooldown: Date.now()})
               message.reply(`Você deu uma reputação para \`${mencao.user.username}\` agora ele tem ${membro.reps + 1} reputação`)
                if(membro.reps + 1 === 50) {
                    ...
                    const coins = await db.coins.findOne({id: mencao.id})
                    await db.reps.findOneAndUpdate({id: mencao.id}, {reps: membro.reps + rep,membro: !message.author ? message.user.id:message.author.id})
                    await db.repsc.findOneAndUpdate({membro: !message.author ? message.user.id:message.author.id, cooldown: Date.now()})
                    if(!coins) return;
                   return await db.coins.findOneAndUpdate({id: mencao.id}, {coinsc: coins.coinsc + 20})
                }  else if (membro.reps + 1 === 100) {
                    message.reply(`Você deu uma reputação para ${mencao} agora ele tem ${membro.reps + 1} reputação e ganhou 5000 koins!`)
            ...
            if(mencao.id === !message.author ? message.user.id:message.author.id) return message.reply('You cannot reputation for yourself!')
            let rep = message.mentions.members.size
