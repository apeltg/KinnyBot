...
module.exports = {
    ...
    run: async (client, message, args) => {
        const vit = message.mentions?.users.first() || client.users.cache.get(!args[0] ? message.options?.getString('user')?.replace(/[<@!>]/g, ''):args[0]) || client.users.fetch(!args[0] ? message.options?.getString('usuario')?.replace(/[<@!>]/g, ''):args[0]);

        if(vit instanceof Promise) {
            await vit.then(u => vit = u).catch(err => vit = !message.author ? message.user:message.author);
        }
        ...
        if (autor.robdown + timeout > Date.now()) {
            ...
            if (vitima) {
                ...
                console.log(autor)
                let athor = !message.author ? message.user.id:message.author.id
                ...
                if (['395995293436477442'].includes(vit.id)) {
                 await db.coins.updateOne({id: !message.author ? message.user.id:message.author.id}, {robdown: Date.now()})
                    ...
                    await db.coins.findOneAndUpdate({id: !message.author ? message.user.id:message.author.id}, {coinsc: autor.coinsc - grana, coinsb: autor.coinsb - grana})
                    message.reply({embeds: [embed]})
                } else {
                    if (vitima && autor) {
                        if (grana <= vitima.coinsc) {
                            await db.coins.findOneAndUpdate({id: vit.id}, {coinsc: vitima.coinsc - grana})
                            await db.coins.findOneAndUpdate({id: !message.author ? message.user.id:message.author.id}, {coinsc: autor.coinsc + grana})
                            await db.coins.updateOne({id: !message.author ? message.user.id:message.author.id}, {robdown: Date.now()})
                            const embed = new MessageEmbed()
                                .setColor('#9900f8')
