...
module.exports = {
    ...
    run: async(client, message, args ) => {
        ...
        const lan = await db.lgs.findOne({guildID: message.author?.id ?? message.user.id})
        if(!args[0]) {
        ...
        message.reply({embeds: [embed]})
            ...
            let espectadores = client["comandos"+message.author.id] ? client["comandos"+message.author.id] += 1 : client["comandos"+message.author.id] = 1
            let materias = []
            let langa = lan && lan.lang === 'en' ? filtroen:filtropt
            langa.map(x => x.materias).forEach(j => j.forEach(m => {
              if (m.titulo === args.join(" ")) materias.push(m)
            }))
            ...
            if(arrays.some(x => message.author.id.includes(x))) return message.reply('Você já esta espectando')
            dba.add(`jornal-${materia.jornal}.espec`, 1);
            dba.push(`jornal-${materia.jornal}.especa`, message.author.id)
            const embed = new MessageEmbed()
            .setColor('#9900f8')
            .setTitle(`${client.user.username} - ${lan && lan.lang == 'en' ? "Newspaper":"Jornal"}`)
            ...
            .setFooter(`Você está espectando essa materia! para não espectar ela clique no ❌ Agora tem ${dba.get(`jornal-${materia.jornal}.espec`)} ${dba.get(`jornal-${materia.jornal}.espec`) == 1 ? "espectador":"espectadores"}`)
